<script>
	...
	const cyberSecurityBlogs = $derived(
		blogs.filter(
			(blog) =>
				blog.categoryType &&
				blog.categoryType.toLowerCase() === 'cyber security'
		)
	);
...
<Seo
	type="WebPage"
	title="Cyber Security — Stay Safe Online | DigitalDSA"
	description="Learn how to protect yourself from cyber threats, phishing scams, and online fraud. Expert cyber security advice for individuals and businesses."
	keywords="cyber security, online safety, phishing scams, data protection, internet security"
/>
...
<SecondPageLayout
	pageData={{
		coverImage: '/images/cyber-security-cover.jpg',
		coverAlt: 'Cyber security and online safety',
...
>
	...
	>
		{#each cyberSecurityBlogs as blog}
			<a
				href={blog.path}
				onclick={(e) => {
					e.preventDefault();
