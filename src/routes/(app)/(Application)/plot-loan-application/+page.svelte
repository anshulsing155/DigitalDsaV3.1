<script lang="ts">
	...
	const config = LOAN_APP_CONFIGS.plot;
...
>
	{#if selectedBankOffer}
		...
		>
			<!-- Offer summary banner — conditional BT/top-up display -->
			...
			<form method="POST" onsubmit={handleSubmit} class="p-4 sm:p-6">
				<div class="space-y-6">
					...
					<!-- Employment + Loan Details shown only for New Loan (not BT) -->
					{#if userData.loanType == 'New Loan'}
						<div>
							<h3 class="text-sectionHeadingText mb-4 border-b border-grayTwo pb-2 text-blue-900">
								Employment Details
							</h3>
