<!--
	...
	Was: src/routes/(app)/(offers)/balance-transfer-offers/+page.svelte

	Reason: this page rendered offers read from
	`localStorage.getItem('balanceTransferOffers')`. Nothing in the
	post-rule-engine codebase wrote that storage key — the only writer had
	been `submitBalanceTransferApplication()` in homeLoanApi.ts, which was
	itself dead code (never called). So this page permanently rendered the
	empty/fallback state on every visit.

	No code in the tree navigated to `/balance-transfer-offers` either —
	routes.ts defined `OFFERS.BALANCE_TRANSFER = '/balance-transfer-offers'`
	as a URL constant but no `goto(ROUTES.OFFERS.BALANCE_TRANSFER)` call
	site existed. The route was unreachable.
	...
	Restore path:
	  git show <pre-S214-sha>:src/routes/(app)/(offers)/balance-transfer-offers/+page.svelte
