<script lang="ts">
	...
	let appliedOffers: { bankName: string; applied: boolean }[] = $state([]);
	...
	function handleApply(offerIndex: number): void {
		...
		goto(`/personal-loan-application?offer=${originalIndex}`);
	}

	onMount(async () => {
		const storedOffers = safeLocalStorage.getItem('personalLoanOffers');
		if (storedOffers) {
			try {
	...
	function backFunction(): void {
		...
		goto(ROUTES.FORM.UNSECURE_LOAN.PERSONAL);
	}
</script>

<Seo
	title="Personal Loan Offers | DigitalDSA"
	description="View your personalized Personal Loan offers from multiple banks with competitive interest rates and flexible terms."
/>
			...
			<div class="mx-auto max-w-7xl px-4 py-4 lg:px-8">
				<div class="grid items-start gap-4 sm:grid-cols-3">
					<div class="sm:col-span-2">
						<h1 class="text-subTitleText text-primaryText">Personal Loan Offers</h1>
			...
			{:else if allOffers.data.length === 0}
				<div class="py-12 text-center">
					...
					<p class="smallText mb-6 text-[var(--form-text-secondary)]">
						Complete the personal loan form to get personalized offers
					</p>
					<button
						onclick={() => goto(ROUTES.FORM.UNSECURE_LOAN.PERSONAL)}
											...
											{#if offer.principalOutstandingDetail.emiOfPrincipalOutstanding}
												<div class="text-end md:text-start">
													...
													>
														...
														{formatCurrency(
															offer.principalOutstandingDetail.emiOfPrincipalOutstanding
														)}
													</div>
												</div>
											{/if}
										...
										<div class="mb-6 grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
											...
											{#if offer.topDetails.emiOfTopUp}
												<div class="text-end md:text-start">
													...
													>
														<IndianRupee size={15} class="inline-block" />
														{formatCurrency(offer.topDetails.emiOfTopUp)}
													</div>
												</div>
											{/if}
