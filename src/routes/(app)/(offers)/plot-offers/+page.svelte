<script lang="ts">
	...
	function handleApply(offerIndex: number): void {
		...
		goto(`/plot-loan-application?offer=${originalIndex}`);
	}

	onMount(async () => {
		// const sessionUser = localStorage.getItem('dsa_session_user');
		// if (sessionUser) {
		...
		const storedOffers = safeLocalStorage.getItem('plotLoanOffers');
	...
	$effect(() => {
		if (allOffers?.data?.length) {
			...
			// 		(o) => o.bankName && String(o.bankName).trim().toLowerCase() === targetBank
			// 	);
	...
	function backFunction(): void {
		...
		goto(ROUTES.FORM.PLOT_LOAN);
	}
</script>

<Seo
	title="Plot Offers | DigitalDSA"
	description="View your personalized Plot Loan offers from multiple banks with competitive interest rates and flexible terms."
/>
			...
			<div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
				<div class="grid items-start gap-4 sm:grid-cols-3">
					<div class="sm:col-span-2">
						<h1 class="text-subTitleText text-primaryText">Plot Loan Offers</h1>
					...
					{#each filteredOffers as offer, index}
						...
						>
							<div class="flex flex-col p-4">
								...
								<!-- sanction amount details -->
								<div class="flex flex-col gap-2">
									{#if formState.applicationData.loanType === 'Balance Transfer Only'}
										<h2
											class="text-labelText {isEligible(offer)
									...
									<div class="mb-6 grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
										<div class="text-start">
											...
											>
												...
												{formatCurrency(
													Math.round(
														(offer.loanData?.plotLoan?.loanAmount ||
															offer.loanData?.plotLoan?.principalOutstanding ||
															0) / 10
													) * 10
												)}
											</div>
										...
										<div class="text-end md:text-start">
											...
											>
												...
												{formatCurrency(
													Math.round(
														offer.loanData?.plotLoan?.emi ||
															offer.loanData?.plotLoan?.emiOfPrincipalOutstanding ||
															0
													)
												)}
											</div>
										...
										<div class="text-start">
											...
											>
												...
												{offer.loanData?.plotLoan?.tenure || 'N/A'}Y
											</div>
										</div>
										<div class="text-end md:text-start">
											<div
											...
											>
												...
												{Number(
													offer.loanData?.plotLoan?.interestRate ||
														offer.loanData?.plotLoan?.annualRate ||
														0
												).toFixed(2)}%
											</div>
										</div>
								...
								<!-- top up details -->
								{#if offer?.loanData?.topUp?.topUpAmount > 0 && offer?.loanData?.topUp?.emiOfTopUp > 0}
									<div class="flex flex-col gap-2">
										<h2
											class="text-labelText {isEligible(offer)
												? 'font-titleBold text-primaryText underline decoration-primary underline-offset-4'
										...
										<div class="mb-6 grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
											{#if offer?.loanData?.topUp?.topUpAmount}
												<div class="text-start">
													<div
														class="smallText uppercase {isEligible(offer)
															? 'text-primaryText'
													...
													>
														<IndianRupee size={16} class="inline-block" />{formatCurrency(
															offer?.loanData?.topUp?.topUpAmount
														)}
													</div>
												</div>
											{/if}

											{#if offer?.loanData?.topUp?.emiOfTopUp}
												<div class="text-end md:text-start">
													<div
														class="smallText uppercase {isEligible(offer)
															? 'text-primaryText'
													...
													>
														<IndianRupee size={16} class="inline-block" />
														{formatCurrency(Math.round(offer?.loanData?.topUp?.emiOfTopUp))}
													</div>
												</div>
											{/if}

											{#if offer?.loanData?.topUp?.tenure}
												<div class="text-start">
													<div
														class="smallText uppercase {isEligible(offer)
															? 'text-primaryText'
													...
													<div
														class="text-sectionHeadingText {isEligible(offer)
															...
															: 'text-[var(--form-text-secondary)]'}"
													>
														<Calendar size={16} class="mr-2 inline-block" />{offer?.loanData?.topUp
															?.tenure || 'N/A'} Y
													</div>
												</div>
											{/if}

											{#if offer?.loanData?.topUp?.annualRate}
												<div class="text-end md:text-start">
													<div
														class="smallText uppercase {isEligible(offer)
															? 'text-primaryText'
													...
													<div
														class="text-sectionHeadingText {isEligible(offer)
															...
															: 'text-[var(--form-text-secondary)]'}"
													>
														<Percent size={16} class="mr-2 inline-block" />{offer?.loanData?.topUp
															?.annualRate || 'N/A'}%
													</div>
												</div>
											{/if}
								...
								{#if offer?.loanData?.plotLoan?.deposit !== offer?.loanData?.plotLoan?.requiredDeposit}
									<div
									...
									>
										...
										<div class=" grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3">
											<div class="text-start">
												...
												>
													<IndianRupee size={16} class="inline-block" />
													{formatCurrency(Math.round(offer?.loanData?.plotLoan?.deposit || 0))}
												</div>
											</div>
											...
											<div class="text-end md:text-center">
												...
												>
													<IndianRupee size={16} class="inline-block" />
													{formatCurrency(Math.round(offer?.loanData?.plotLoan?.requiredDeposit))}
												</div>
											</div>
											{#if offer?.loanData?.plotLoan?.shortDownPayment}
												<div class="text-start md:text-end">
													...
													>
														<IndianRupee size={16} class="inline-block" />
														{formatCurrency(
															Math.abs(Math.round(offer?.loanData?.plotLoan?.shortDownPayment))
														)}
													</div>
												</div>
											{/if}
									...
									{#if offer?.loanData?.personalLoanData?.length}
										<div class="min-w-full border-t border-dashed border-primary py-4">
											...
											<div class="mb-6 flex items-center gap-2">
												<h2
													class="text-labelText pl-4 {isEligible(offer)
														? 'font-titleBold text-primaryText underline decoration-primary underline-offset-4'
														: 'text-[var(--form-text-secondary)]'}"
												>
													Extra Loan Details
												...
												<table class="min-w-full overflow-hidden border border-primary">
													...
													>
														<tr>
															...
															<th class="px-4 py-2 text-left text-nowrap uppercase">Loan Type</th>
															<th class="px-4 py-2 text-left text-nowrap uppercase">Bank Name</th>
															<th class="px-4 py-2 text-left text-nowrap uppercase">Loan Amount</th>
															<th class="px-4 py-2 text-left text-nowrap uppercase">Tenure</th>
															<th class="px-4 py-2 text-left text-nowrap uppercase">EMI</th>
															<th class="px-4 py-2 text-left text-nowrap uppercase"
																>Interest Rate</th
															>
														</tr>
													</thead>
														...
														{#each offer.loanData.personalLoanData as personalLoanData}
															{#if personalLoanData.emi > 0 && personalLoanData.loanAmount > 0}
																<tr class={isEligible(offer) ? 'bg-primary/5' : 'opacity-70'}>
																	<!-- Applicant -->
																	<td
																		class="smallText px-4 py-2 text-nowrap text-[var(--form-text)]"
																	>
																		<!-- <UserRoundCheck size={16} class="inline-block" /> -->
																		{personalLoanData.applicantName ||
																			formState.applicationData.allApplicantDetails[0].companyName}
																	</td>
																	...
																	<!-- Interest Rate -->
																	<td class="smallText px-4 py-2 text-[var(--form-text)]">
