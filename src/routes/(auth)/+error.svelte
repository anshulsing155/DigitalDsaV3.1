<script lang="ts">
	// Route-group error boundary for /(auth)/* (login, partner-signup).
	// Catches errors thrown by load functions or +page.svelte mount logic inside
	// the (auth) group, so a crash in login/signup keeps the user on a friendly
	// auth-context page instead of bubbling to the root error page.
	//
	// Mirrors src/routes/(app)/+error.svelte. See CODE-REVIEW-2026-05-22-full.md §F5.
	...
	$effect(() => {
		...
		clientLogger.error(
			...
			`(auth) route error ${determinedStatus}`
		);
	});
