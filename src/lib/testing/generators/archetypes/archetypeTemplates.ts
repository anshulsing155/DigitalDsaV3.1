...
export interface ArchetypeTemplate {
	...
	loanType: string;
	/**
	 * Plot Loan variant (`Plot Loan Only` / `Plot & Construction Loan` /
	 * `Plot & Equity Loan` / `Construction Loan Only`). Optional — only Plot
	 * Loan archetypes set this. Added S210 audit per ADR-0020 four-field model;
	 * pre-rename, variant values were incorrectly placed on `loanType`.
	 */
	loanVariant?: string;
...
const PLOT_LOAN_ARCHETYPES: ArchetypeTemplate[] = [
	...
	// --- Plot Balance Transfer ---
	{
		...
		loanName: 'Plot Loan',
		// Plot BT scope; no variant question is asked for BT in Plot. Pre-rename
		// this incorrectly carried `loanType: 'Plot Balance Transfer'` — a non-
		// canonical hybrid string. Fixed S210 audit per ADR-0020.
		loanType: 'Balance Transfer Only',
		employmentType: 'Salaried(Private)',
		propertyType: 'Plot',
		cityHint: 'Guwahati',
