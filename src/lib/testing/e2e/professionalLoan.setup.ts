/**
 * Professional Loan E2E Test Setup
 */
...
export const ROUTES = {
	...
	PROFESSIONAL_FORM: APP_ROUTES.FORM.UNSECURE_LOAN.PROFESSIONAL
};

export async function navigateToProfessionalLoan(page: Page, loanType: string = 'New Loan') {
	await navigateToLoanForm(
		page,
		'Professional Loan',
		APP_ROUTES.FORM.UNSECURE_LOAN.PROFESSIONAL,
		loanType
	);
}
...
export async function fillEligibilityCheckPage(page: Page) {
	...
	const nriVisible = await isQuestionVisible(page, 'q4_applicantIsNRI');
	if (nriVisible) {
		await selectRadio(page, 'q4_applicantIsNRI', 'No');
	}
}

/** Fill locationPage — practice state/city, professional banks */
export async function fillProfessionalLocationPage(page: Page) {
