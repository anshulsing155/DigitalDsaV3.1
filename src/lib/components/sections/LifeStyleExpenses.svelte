<script lang="ts">
  ...
  let InvestmentsAndSavings = JSON.parse(JSON.stringify(budgetConfig.lifestyleExpenses.InvestmentsAndSavings));
  let CreditAndloans = JSON.parse(JSON.stringify(budgetConfig.lifestyleExpenses.CreditAndloans));
  let HealthAndfitness = JSON.parse(JSON.stringify(budgetConfig.lifestyleExpenses.HealthAndfitness));
  let Incidentals = JSON.parse(JSON.stringify(budgetConfig.lifestyleExpenses.Incidentals));
  let Gift = JSON.parse(JSON.stringify(budgetConfig.lifestyleExpenses.Gift));

  $effect(() => {
    if ($budgetData) {
      if (!$budgetData.lifeStyleDataArray || $budgetData.lifeStyleDataArray.length === 0) {
        $budgetData.lifeStyleDataArray = [{
          "Investment & savings": InvestmentsAndSavings,
          "Credit & loans": CreditAndloans,
              ...
              <div class="grid xs:grid-cols-2 sm:justify-center items-center gap-4 border-b pb-6 border-gray-300">
                ...
                <div class="grid grid-cols-2 gap-4 font-Paragraph text-minParaFont md:text-paraFont">
                  ...
                  <Select
                    ...
                    onChange={() => {
                      $budgetData.lifeStyleDataArray = [...$budgetData.lifeStyleDataArray];
                    }}
                  />
                </div>
              </div>
