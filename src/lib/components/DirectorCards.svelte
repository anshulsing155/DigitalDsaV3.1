<script lang="ts">
		...
		for (let i = 0; i < formState.applicants.length; i++) {
			...
			if (aName === normalized) {
				...
				if (a.isNRI !== undefined && a.isNRI !== null) {
					const nriVal = a.isNRI as unknown;
	...
	function syncDirectorsToFormState(companyId: string) {
		...
		for (const d of directors) {
			...
			const existingIdx = result.findIndex((a) => {
				...
				// Honor user's "different applicant" declaration — never merge a
				// standalone Individual that was explicitly marked independent of
				// same-named records. Without this guard the standalone gets
				// overwritten with director data (including linkedCompanyId),
				// disappears from the Who's Applying table, and the visible row
				// count silently drifts below applicants.length.
				if (a.__independentOfSameName) return false;
		...
		for (const company of companyApplicants) {
			...
			for (const d of directors) {
				...
				const existingIdx = result.findIndex((a) => {
					if (a.applicantType !== 'Individual') return false;
					if (a.id === d.id) return true;
					// Same guard as syncDirectorsToFormState — see comment there.
					if (a.__independentOfSameName) return false;
					const applicantName = normalizeName((a.fullName as string) ?? '');
					return applicantName !== '' && applicantName === directorName;
				});
	...
	function clearDirector(companyId: string, idx: number) {
		...
		const company = companyApplicants.find((c) => c.id === companyId);
				...
				for (let i = 0; i < directorCount; i++) {
					...
					if (saved && saved.fullName) {
						forms.push({
							...
							restoredFrom: '',
							lockedFields: isOPC ? ['ownershipPercent'] : [],
							pendingMatch: null
						});
					} else {
						forms.push({
...
<div class="flex flex-col gap-12 pb-4">
	...
	<div class="flex items-start gap-3">
		...
		<div class="flex-1">
			<h3 class="text-sectionHeadingText font-semibold text-gray-800 dark:text-gray-100">
				Director & Partner Details
			</h3>
			<p class="mt-1 text-xs leading-relaxed text-gray-500 dark:text-gray-400">
				Fill in details for each member one at a time. Confirm each entry before moving to the next.
				{isUnsecured
					? 'Provide basic profile details for each member.'
					: 'Those on property or EMI will automatically become co-applicants.'}
			</p>
		</div>
	</div>
						...
						>
							...
							<div class="grid grid-cols-2 gap-3 md:grid-cols-3">
								...
								<div class="col-span-2 md:col-span-1">
									<label
										for="director-{companyId}-{idx}-fullName"
										class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400"
									>
										Full Name *
									</label>
									<input
										id="director-{companyId}-{idx}-fullName"
										type="text"
								...
								<div class="col-span-1">
									...
									<select
										...
										class="form-select w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-800
											...
											? 'cursor-not-allowed !bg-gray-100 !text-gray-500 dark:!bg-gray-800 dark:!text-gray-300'
											: ''}
							...
							<div class="mt-3 grid grid-cols-2 gap-3 md:grid-cols-3">
								...
								<div class="col-span-1">
									<label
										for="director-{companyId}-{idx}-ownership"
										class="mb-1 flex items-center gap-1 text-xs font-medium text-gray-600 dark:text-gray-400"
									>
										Ownership % *
										{#if isLocked(director, 'ownershipPercent')}<Lock
												class="h-2.5 w-2.5 text-blue-400"
											/>{/if}
									</label>
									<input
										id="director-{companyId}-{idx}-ownership"
										type="text"
										bind:value={director.ownershipPercent}
										disabled={isLocked(director, 'ownershipPercent')}
										placeholder="%"
										maxlength={3}
										class="form-input w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-800 placeholder:text-gray-400
											focus:border-stone-500 focus:ring-1 focus:ring-stone-500
											dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100
											{isLocked(director, 'ownershipPercent')
											? 'cursor-not-allowed !bg-gray-100 !text-gray-500 dark:!bg-gray-800 dark:!text-gray-300'
											: ''}
								...
								{#if !isUnsecured}
									<div class="col-span-1">
										<label
											for="director-{companyId}-{idx}-location"
											class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400"
										>
											Location *
										</label>
										<select
											id="director-{companyId}-{idx}-location"
								...
								{#if isUnsecured}
									<div class="col-span-1">
										...
										>
											Is NRI? *
											{#if isLocked(director, 'isNRI')}<Lock
													class="h-2.5 w-2.5 text-blue-400"
												/>{/if}
										</label>
										<select
							...
							{#if !isUnsecured}
								<div class="mt-3 grid grid-cols-3 gap-3">
									...
									<div class="col-span-1">
										...
										<select
											...
											{isLocked(director, 'onProperty')
												? 'cursor-not-allowed border-gray-300 !bg-gray-100 !text-gray-500 dark:border-gray-600 dark:!bg-gray-800 dark:!text-gray-300'
									...
									<div class="col-span-1">
										...
										>
											Will Pay EMI? *
											{#if isLocked(director, 'onEMI')}<Lock
													class="h-2.5 w-2.5 text-blue-400"
												/>{/if}
										</label>
										<select
