...
**Item 1 — ApplicantSelect placeholder muted (`13d72d7f`).** Follow-up to the morning's visual-unification commit. The user-supplied refresh added `text-[var(--form-text)]` to the value `<span>` unconditionally — Tailwind utility specificity defeats the `.app-select-placeholder { color: var(--form-text-muted) }` rule via load order, so placeholder text rendered in solid black/white instead of muted gray. Fix: gate the utilities behind `class:text-black={selectedOption}` + `class:dark:text-white={selectedOption}` so placeholder state falls through to the muted CSS. One-line conditional, no test (presentation logic).
