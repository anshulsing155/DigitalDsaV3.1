...
Object.entries(schemas).forEach(([name, schema]) => {
  ...
  const hasShowWhenLoanType = allQuestions.filter(q => {
    const json = JSON.stringify(q);
    return json.includes('loanType');
  });
  console.log(`${name}: ${hasShowWhenLoanType.length} questions with loanType conditions`);
  const variantCounts = {};
  ...
  console.log('  Variants:', variantCounts);
  console.log('');
});
